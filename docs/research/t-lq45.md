...
- **Method caveat, for Shyden.** IDX's Terms of Use (quoted in §5) say *"tidak diperkenankan menggunakan metode web scrapping/crawling"*. This research sent about 45 scripted requests to IDX's announcement API and PDF paths from a browser session. T-RULES and T-PAY used the same technique. It stopped as soon as the clause was read. **Shyden's decision (2026-09-25):** future research uses another source first (Internet Archive copies, KSEI, BPK), and falls back to this technique only when no other source has the document.
